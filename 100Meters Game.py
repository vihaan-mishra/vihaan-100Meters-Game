...
class Decath100MFrame(Frame):
    ...
    def roll(self):
        ...
        # roll four dice
        start_box = self.gameround * 4
        for i in range(start_box, start_box + 4):
            self.dice[i].roll()
    ...
    def keep(self):
        ...
        if self.gameround < 2:  # move buttons to next four of dice
            self.rollButton.grid(row=2, column=5 * self.gameround, columnspan=2)
            self.keepButton.grid(row=3, column=5 * self.gameround, columnspan=2)
